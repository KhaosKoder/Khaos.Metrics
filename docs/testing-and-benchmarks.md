...
Aim for 80%+ in `Khaos.Metrics.Core`, especially for non-trivial helpers such as `MonitoringSnapshotProvider`, `OperationTags`, `MonitoringServiceCollectionExtensions`, and `EventDispatcher`.

## Benchmarks

The `benchmarks/Khaos.Metrics.Benchmarks` project uses BenchmarkDotNet to track instrumentation overhead. Typical runs:

```powershell
dotnet run -c Release --project benchmarks/Khaos.Metrics.Benchmarks
```
