# Khaos.Metrics - Final Design Specification (v1)

## 1. Scope

Khaos.Metrics is a .NET library that:
...
- **Khaos.Metrics.Core** - target `net8.0`.
- **Khaos.Metrics.AspNet** - target `net8.0`.
- Optional future packages (for example, OpenTelemetry integration) also target `net8.0`.
...
- **Optional integrations** (separate packages, not part of v1 core):
  - `Khaos.Metrics.OpenTelemetry` may depend on `OpenTelemetry`, `OpenTelemetry.Metrics`, or `System.Diagnostics.Metrics`.
- **Tests/Benchmarks**
...
{
    public static IServiceCollection AddKhaosMetrics(
...
- No additional factory or registration helpers are required beyond `AddKhaosMetrics`.
...
- Coverage target: at least 80% line coverage for `Khaos.Metrics.Core` (excluding trivial properties/constructors).
